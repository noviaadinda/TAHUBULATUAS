...
def tambah():
    conn = sqlite3.connect('cars.db')
    c = conn.cursor()
    c.execute("""INSERT INTO cars (ID, BRAND, MODEL, PRICE) VALUES ("150", "DAIHATSU", "GRANMAX", "130")""")
    conn.commit()
    c.close()
...
def tambah():
    conn = sqlite3.connect('cars.db')
    c = conn.cursor()
    c.execute("""UPDATE cars SET MODEL='Crv' WHERE BRAND='HONDA' """)
    conn.commit()
    c.close()
...
def tambah():
    conn = sqlite3.connect('cars.db')
    c = conn.cursor()
    c.execute("""DELETE FROM cars WHERE ID='150'""")
    conn.commit()
    c.close()
